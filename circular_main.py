...
def eval_replay_circularly(args):
    ...
    axis.set_yticklabels(['0','','','','0.8','1.0'], fontdict={'size':40})
    axis.set_xticks(x, ['Pre', '', 'Learning', '', 'Post'], fontdict={'size':40})
...
def Ablation_KL_div(args):
    n_seed = 3
    title_list = ['origin', 'dup 5 to all']
    # color_list = ['lightcoral', 'darkgreen', 'navy']
    color_list = ['lightcoral', 'darkgreen']
    ...
    args.prefix = 'hml_926_duplicate_5_to_all'
    line_chart_list = []
    step_count_list = [[] for _ in range(5)]
    for i in range(n_seed):
        args.initkey = i
    ...
    # args.prefix = 'hml_926_zero_toHPC'
    # line_chart_list = []
    # step_count_list = [[] for _ in range(5)]
    # for i in range(n_seed):
    #     args.initkey = i
    ...
    print('noreplay:',step_count_all[1])
    step_count_mean = [x.mean() for x in step_count_all]
    step_count_std = [x.std() for x in step_count_all]
    ...
    for j in range(len(step_count_mean)):
        axes[1].errorbar(jnp.arange(len(step_count_mean))[j], step_count_mean[j], step_count_std[j], fmt='-', capsize=8,
                        markersize=10, linewidth=4, capthick=4, c=color_list[j])
        axes[1].scatter(jnp.arange(len(step_count_mean))[j], step_count_mean[j], s=100, c=color_list[j])
    axes[1].spines['top'].set_color('none')
    axes[1].spines['right'].set_color('none')
    axes[1].spines['bottom'].set_linewidth(3)
    axes[1].spines['left'].set_linewidth(3)
    axes[1].set_xticks(jnp.arange(len(step_count_mean)),title_list, fontdict={'size':30}, rotation=45)
